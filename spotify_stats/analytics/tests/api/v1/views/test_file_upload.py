...
def mock_process_file_upload_jobs(mocker):
    return mocker.patch(
        "spotify_stats.analytics.api.v1.views.file_upload.process_file_upload_jobs"
    )
...
class TestUserFileUploadJobListCreateView:
    ...
    def test_anonymous_user(self, api_client, file_upload_url, method, expected_status):
        response = getattr(api_client, method)(file_upload_url)
        assert response.status_code == expected_status

    # GET
    def test_lists_only_request_user_jobs(
        self, api_client, file_upload_url, user, file_upload_job_factory
    ):
        ...
        response = api_client.get(file_upload_url)
    ...
    # POST
    def test_error_if_file_size_exceeds_max_file_size(
        self, api_client, file_upload_url, user
    ):
        api_client.force_authenticate(user=user)
    ...
    def test_error_if_no_files_provided(self, api_client, file_upload_url, user):
        api_client.force_authenticate(user=user)
        response = api_client.post(file_upload_url, data={"files": []})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.data["files"][0].code == "required"

    def test_successful_single_file_upload(
        self, api_client, file_upload_url, user, fake, mock_process_file_upload_jobs
    ):
        api_client.force_authenticate(user=user)
