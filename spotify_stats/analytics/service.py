from django.db.models import Sum, Count, FloatField, Avg, Min, Max
...
class StreamingAnalyticsService:
    ...
    @staticmethod
    def top_artists(base_queryset):
        return (
            Artist.objects
            .filter(tracks__history__in=base_queryset)
            .annotate(
    ...
    def top_tracks(base_queryset):
        return (
            Track.objects
            .filter(history__in=base_queryset)
    ...
    def listening_stats(base_queryset):
        return base_queryset.aggregate(
            ...
            average_mins_played=Coalesce(
                Round(
                    Cast(Avg("ms_played"), FloatField()) / 1000 / 60,
                    2
                ),
                0,
                output_field=FloatField()
    ...
    @staticmethod
    def yearly_activity(base_queryset):
        return (
            base_queryset
            .annotate(year=TruncYear("played_at"))
            .values("year")
            .annotate(
    ...
    @staticmethod
    def monthly_activity(base_queryset):
        return (
            base_queryset
    ...
    @staticmethod
    def daily_activity(base_queryset):
        return (
            base_queryset
